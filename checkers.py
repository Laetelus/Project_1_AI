...
class Checker(TwoPlayerGame):
    def __init__(self, players):
        ...
        self.black_pieces = [(0, 1), (0, 3), (0, 5), (0, 7), (1, 0), (1, 2),
                             (1, 4), (1, 6)]
        #assign row and columns to the white_pieces
        self.white_pieces = [(6, 1), (6, 3), (6, 5), (6, 7), (7, 0), (7, 2),
                             (7, 4), (7, 6)]
    ...
    def possible_moves_on_white_turn(self):
            ...
            for n in step_pos:
                if (n[0] >= 0 and n[0] <= 7) and (
                    ...
                    if board[n[0], n[1]] in ["B", "W"]:
                        ...
                        if is_inside_board and (
                                j in black_squares) and is_position_empty:
        ...
        for i, j in old_new_piece_pos:
            ...
            assert len(
                np.where(b != 0)[0]
            ) == 16, f"In possible_moves_on_white_turn(), there are {len(np.where(b != 0)[0])} pieces on the board  \n {b}"
    ...
    def possible_moves_on_black_turn(self):
        ...
        for i, j in old_new_piece_pos:
            ...
            assert len(
                np.where(b != 0)[0]
            ) == 16, f"In possible_moves_on_black_turn(), there are {len(np.where(b != 0)[0])} pieces on the board  \n {b}"
    ...
    def get_piece_pos_from_table(self, table_pos):
        ...
        assert len(
            np.where(table_pos != 0)[0]
        ) == 16, f"In get_piece_pos_from_table(), there are {len(np.where(table_pos != 0)[0])} pieces on the board  \n {table_pos}"
    ...
    def make_move(self, pos):
        ...
        """
        #matts code start
        if self.current_player == 1:  # if current player is white player or player 1
            self.players[0].pos = self.get_piece_pos_from_table(pos)
            return self.players[0].pos
        else:  # if black player or player 2
